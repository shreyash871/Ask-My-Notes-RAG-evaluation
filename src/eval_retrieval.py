...
def score(retrieved_ids: list[int], gold: set[int], k: int) -> dict:
    top = retrieved_ids[:k]
    found = [cid for cid in top if cid in gold]
    hit = 1.0 if found else 0.0
    recall = len(found) / min(k, len(gold))        # capped denominator
    rr = 0.0
...
def main():
    ...
    per_q = []
    for row in rows:
        hits = r.retrieve(row["question"], k=K)
    ...
    for c, s in report["by_company"].items():
        print(f"  {c:8} {s}")
    print("\nMISSES (no gold chunk in top-k)")
    for q in per_q:
        if q["hit"] == 0:
            print(f"  {q['id']}  {q['question'][:70]}")

    out = RESULTS / f"retrieval_cs{r.config['chunk_size']}_k{K}.json"
